 ...
 */
function doDepartment(department, operate) {
  operate(department);
  if (department.department || department.department.length > 0) {
    for (var i = 0; i < department.department.length; i++) {
 ...
 */
function cloneDepartmentTree(company) {
  var clone = {
    _id: company._id,
    name: company.info.name,
    level:0
 ...
 */
function sendDepartments(company, department_ids, res) {
  Department
...
function departmentOperate(options, callback){
  var did = options.did;
